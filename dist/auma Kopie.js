...
module.exports = {
  ...
  waveforms: {
    ...
    * These variables make auma run. The config is the most important
    * containing active settings and parameters.
    *
    * The config JSON is the global settings for ALL of auma functions.
    ...
    * @module config
    * @type {object}
    * @property {string}  	config.version          				- The current version of aumaJS.
    ...
    * @property {array} 		config.songs										- Contains all of the songs the user has passed to auma to use.
    * @property {object} 	config.playlists								- Contains all of the playlists the user created.
    * @property {object} 	config.start_song 							- The index of the song that aumaJS should start with.
    * @property {string} 	config.starting_playlist 				- The starting playlist the player will intiialize to.
    * @property {string} 	config.starting_playlist_song 	- The index of the song in the playlist that should be started.
    * @property {boolean} 	config.repeat 									- When repeat is on, when the song ends the song will replay itself.
    * @property {object} 	config.shuffle_list							- When shuffled, gets populated with the songs the user provided in a random order.
    * @property {boolean} 	config.shuffle_on								- When on, gets set to true so when traversing through songs, aumaJS knows whether or not to use the songs object or the shuffle_list
    ...
    * @property {boolean} 	config.debug										- When set to true, aumaJS will print to the console any errors providing helpful feedback to the user.
    * @property {number} 	config.volume 									- The user can set the initial volume to a number between 0 and 1 over-riding the default of .5
    * @property {number} 	config.pre_mute_volume 					- This is set on mute so that when a user un-mutes aumaJS knows what to restore the volume to.
    * @property {number}		config.volume_increment 				- The default values are an integer between 1 and 100 for how much the volume should increase when the user presses the volume up button.
    * @property {number}		config.volume_decrement 				- The default values are an integer between 1 and 100 for how much the volume should decrease when the user presses the volume down button.
    * @property {string} 	config.soundcloud_client 				- When using SoundCloud, the user will have to provide their API Client ID
    * @property {boolean} 	config.soundcloud_use_art 			- The user can set this to true and aumaJS will use the album art for the song returned from the Soundcloud API
 ...
 * helpers and visual synchronizers. These are the core functions of aumaJS.
...
var Core = function () {
   ...
   *
   * Public Accessor: auma.setVolume( volumeLevel )
   *
   * @access public
   ...
   * a live source.
   *
   * Public Accessor: auma.setSongLocation( songPercentage )
   *
   * @access public
   ...
   *
   * Public Accessor: auma.skipToLocation( seconds )
   *
   * @access public
    ...
    _config2.default.audio.addEventListener("canplaythrough", function () {
      ...
      */
      if (_config2.default.audio.duration >= seconds && seconds > 0) {
        ...
        _debug2.default.writeMessage("auma can't skip to a location greater than the duration of the audio or less than 0");
      }
    }, { once: true });
  }
   ...
   *
   * Public Accessor: auma.disconnectStream()
   *
   * @access public
   */
   ...
   *
   * Public Accessor: auma.reconnectStream()
   *
   * @access public\
   */
...
/**
 * Imports aumaJS Callback Utility
...
/**
 * Defines the visual representation of aumaJS play pause elements.
 * @module visual/PlayPauseElements
 */
var PlayPauseElements = function () {
  ...
  function syncGlobal() {
    ...
    */
    var playPauseElements = document.querySelectorAll(".auma-play-pause");
    ...
    */
    for (var i = 0; i < playPauseElements.length; i++) {
      ...
      */
      var playlist = playPauseElements[i].getAttribute("data-auma-playlist");
      var song = playPauseElements[i].getAttribute("data-auma-song-index");
  ...
  function syncPlaylist() {
    ...
    */
    var playlistPlayPauseElements = document.querySelectorAll('.auma-play-pause[data-auma-playlist="' + _config2.default.active_playlist + '"]');
    ...
    */
    for (var i = 0; i < playlistPlayPauseElements.length; i++) {
      ...
      */
      var song = playlistPlayPauseElements[i].getAttribute("data-auma-song-index");
  ...
  function syncSong() {
    ...
    /*
      ...
      auma-song-index that matches the active index attribute.
    */
    var songPlayPauseElements = document.querySelectorAll('.auma-play-pause[data-auma-song-index="' + _config2.default.active_index + '"]');
    ...
    */
    for (var i = 0; i < songPlayPauseElements.length; i++) {
      ...
      */
      var playlist = songPlayPauseElements[i].getAttribute("data-auma-playlist");
  ...
  function syncSongInPlaylist() {
    ...
    /*
      Get all of the individual song play pause buttons. These have an
      auma-song-index attribute. Some have auma-playlist which
      means they are individual songs within a playlist.
    */
    var songInPlaylistPlayPauseElements = document.querySelectorAll('.auma-play-pause[data-auma-song-index="' + activePlaylistIndex + '"][data-auma-playlist="' + _config2.default.active_playlist + '"]');
  ...
  /**
   * Sets an element to be playing by removing the 'auma-paused' class
   * and adding the 'auma-playing' class
   *
   * @access public
   * @param {element} element 	- The element getting the playing class added.
   */
  function setElementPlay(element) {
    element.classList.add("auma-playing");
    element.classList.remove("auma-paused");
  }

  /**
   * Sets an element to be paused by adding the 'auma-paused' class
   * and removing the 'auma-playing' class
   *
   * @access public
   * @param {element} element 	- The element getting the paused class added.
   */
  function setElementPause(element) {
    element.classList.remove("auma-playing");
    element.classList.add("auma-paused");
  }
...
/**
 * aumaJS Audio Navigation Utility.
 *
 * @module utilities/AudioNavigation
 */
...
/**
 * Handles the debugging of aumaJS
 * @module utilities/Debug
 */
var Debug = function () {
  /**
  ...
  function writeMessage(message) {
    /*
      If the user has flagged aumaJS to debug, we print out a message
      to the console.
    */
...
/**
 * aumaJS Checks Module. Checks for new songs, albums, and playlists
 *
 * @module utilities/Checks
 */
var Checks = function () {
   ...
   *
   * Public Accessor: aumaHelpers.isURL( url )
   *
   * @access public
   ...
   *
   * Public Accessor: aumaHelpers.isInt( int )
   *
   * @access public
...
var ConfigState = function () {
   ...
   *
   * Public Accessor: aumaHelpers.resetConfig()
   *
   * @access public
   */
...
var MetaDataElements = function () {
  ...
  function displayMetaData() {
    ...
    */
    var songInfoElements = document.querySelectorAll("[data-auma-song-info]");
    ...
    */
    for (var i = 0; i < songInfoElements.length; i++) {
      ...
      key.
      */
      var info = songInfoElements[i].getAttribute("data-auma-song-info");

      /*
      Grab the playlist and song index.
      */
      var playlist = songInfoElements[i].getAttribute("data-auma-playlist");
      var songIndex = songInfoElements[i].getAttribute("data-auma-song-index");
  ...
  function displayPlaylistMetaData() {
    ...
    */
    var playlistInfoElements = document.querySelectorAll("[data-auma-playlist-info]");
    ...
    */
    for (var i = 0; i < playlistInfoElements.length; i++) {
      ...
      key.
      */
      var info = playlistInfoElements[i].getAttribute("data-auma-playlist-info");
      var playlist = playlistInfoElements[i].getAttribute("data-auma-playlist");
  ...
  function setFirstSongInPlaylist(song, playlist) {
    ...
    */
    ...
    /*
      Get all of the song info elements
    */
    var songInfoElements = document.querySelectorAll('[data-auma-song-info][data-auma-playlist="' + playlist + '"]');
    ...
    */
    for (var i = 0; i < songInfoElements.length; i++) {
      ...
      */
      var elementPlaylist = songInfoElements[i].getAttribute("data-auma-playlist");
    ...
    for (var i = 0; i < songInfoElements.length; i++) {
      ...
      if (songIndex != null && playlist == null) {
        ...
        /*
         ...
         we want isn't found.
         i.e.
            data-auma-song-info="cover_art_url" defaults to using the value
      ...
      */
      if (songIndex != null && playlist != null) {
        ...
        */
        var _info = songInfoElements[i].getAttribute("data-auma-song-info");
...
var RepeatElements = function () {
   ...
   * can apply styles to the 'auma-repeat-on' and
   * 'auma-repeat-off' classes. They represent the state
   * of the player.
   */
  function syncRepeat() {
    /*
    ...
    */
    var repeatClasses = document.getElementsByClassName("auma-repeat");

    /*
    Iterate over all of the repeat classes. If repeat is on,
    then add the 'auma-repeat-on' class and remove the
    'auma-repeat-off' class. If it's off, then do the
    opposite.
    */
    for (var i = 0; i < repeatClasses.length; i++) {
      if (_config2.default.repeat) {
        repeatClasses[i].classList.add("auma-repeat-on");
        repeatClasses[i].classList.remove("auma-repeat-off");
      } else {
        repeatClasses[i].classList.remove("auma-repeat-on");
        repeatClasses[i].classList.add("auma-repeat-off");
      }
    }
  }
   ...
   * can apply styles to the `auma-repeat-on` and `auma-repeat-off`
  ...
  function syncRepeatPlaylist(playlist) {
    ...
    */
    var repeatButtons = document.getElementsByClassName("auma-repeat");
    ...
    */
    for (var i = 0; i < repeatButtons.length; i++) {
      ...
      */
      if (repeatButtons[i].getAttribute("data-auma-playlist") == playlist) {
        /*
        If the state of the playlist is shuffled on, true, then
        we add the 'auma-repeat-on' class and remove the
        'auma-repeat-off' class. If the player is not shuffled
        then we do the opposite.
        */
        if (_config2.default.playlists[playlist].repeat) {
          repeatButtons[i].classList.add("auma-repeat-on");
          repeatButtons[i].classList.remove("auma-repeat-off");
        } else {
          repeatButtons[i].classList.add("auma-repeat-off");
          repeatButtons[i].classList.remove("auma-repeat-on");
        }
      }
    }
  }
   ...
   * can apply styles to the 'auma-repeat-song-on' and
   * 'auma-repeat-song-off' classes. They represent the state
   * of the player.
   */
  function syncRepeatSong() {
    ...
    */
    var repeatSongClasses = document.getElementsByClassName("auma-repeat-song");

    /*
    Iterate over all of the repeat song classes. If repeat is on,
    then add the 'auma-repeat-song-on' class and remove the
    'auma-repeat-song-off' class. If it's off, then do the
    opposite.
    */
    for (var i = 0; i < repeatSongClasses.length; i++) {
      if (_config2.default.repeat_song) {
        repeatSongClasses[i].classList.add("auma-repeat-song-on");
        repeatSongClasses[i].classList.remove("auma-repeat-song-off");
      } else {
        repeatSongClasses[i].classList.remove("auma-repeat-song-on");
        repeatSongClasses[i].classList.add("auma-repeat-song-off");
      }
    }
  }
...
var MuteElements = function () {
  ...
  function setMuted(state) {
    ...
    */
    var muteClasses = document.getElementsByClassName("auma-mute");

    /*
    Iterate over all of the mute classes. If the state of the player
    is not-muted then we add the auma-not-muted classe and remove
    the auma muted class otherwise we do the opposite.
    */
    for (var i = 0; i < muteClasses.length; i++) {
      if (!state) {
        muteClasses[i].classList.add("auma-not-muted");
        muteClasses[i].classList.remove("auma-muted");
      } else {
        muteClasses[i].classList.remove("auma-not-muted");
        muteClasses[i].classList.add("auma-muted");
      }
    }
  }
...
var VolumeSliderElements = function () {
   ...
   */
  function sync() {
    var volumeSliders = document.getElementsByClassName("auma-volume-slider");
...
/**
 * aumaJS Repeater utility. Handles setting the repeat for all scenarios.
 *
...
/**
 * aumaJS Shuffle Module. Handles all of the shuffling functionality for
 * aumaJS
 *
...
/**
 * Defines the visual representation of aumaJS song slider elements.
 * @module visual/SongSliderElements
 */
var SongSliderElements = function () {
  ...
  function syncMain(location) {
    ...
    */
    var mainSongSliders = document.querySelectorAll(".auma-song-slider");
    ...
    */
    for (var i = 0; i < mainSongSliders.length; i++) {
      /*
        Grab the playlist and song attributes from the element.
      */
      var playlist = mainSongSliders[i].getAttribute("data-auma-playlist");
      var song = mainSongSliders[i].getAttribute("data-auma-song-index");
  ...
  function syncPlaylist(location, playlist) {
    ...
    */
    var playlistSongSliders = document.querySelectorAll('.auma-song-slider[data-auma-playlist="' + playlist + '"]');
    ...
    */
    for (var i = 0; i < playlistSongSliders.length; i++) {
      /*
        Grab the playlist and song attributes from the element.
      */
      var playlistAttribute = playlistSongSliders[i].getAttribute("data-auma-playlist");
      var songAttribute = playlistSongSliders[i].getAttribute("data-auma-song-index");
  ...
  function syncSong(location, songIndex) {
    ...
    */
    if (_config2.default.active_playlist == null) {
      ...
      */
      var songSliders = document.querySelectorAll('.auma-song-slider[data-auma-song-index="' + songIndex + '"]');
      ...
      */
      for (var i = 0; i < songSliders.length; i++) {
        /*
         Grab the playlist and song attributes from the element.
        */
        var playlistAttribute = songSliders[i].getAttribute("data-auma-playlist");
        var songAttribute = songSliders[i].getAttribute("data-auma-song-index");
  ...
  function syncSongInPlaylist(location, playlist) {
    ...
    */
    var songInPlaylistSliders = document.querySelectorAll('.auma-song-slider[data-auma-playlist="' + playlist + '"][data-auma-song-index="' + activePlaylistIndex + '"]');
  ...
  function resetElements() {
    var songSliders = document.getElementsByClassName("auma-song-slider");
...
/**
 * Imports the aumaJS Duration Second Elements
 * @module visual/time/DurationSecondElements
 */


/**
 * Imports the aumaJS Duration Hour Elements
 * @module visual/time/DurationHourElements
 */


/**
 * Imports the aumaJS Current Second Elements
 * @module visual/time/CurrentTimeElements
 */


/**
 * Imports the aumaJS Current Hour Elements
...
/**
 * Imports the aumaJS Duration Time Elements
 * @module visual/time/DurationTimeElements
 */


/**
 * Imports the aumaJS Duration Minute Elements
 * @module visual/time/DurationMinuteElements
 */


/**
 * Imports the aumaJS Duration Count Down Time Elements
 * @module visual/time/DurationCountDownTimeElements
 */


/**
 * Imports the aumaJS Current Minute Elements
 * @module visual/time/CurrentMinuteElements
 */
/**
 * Imports the aumaJS Current Time
 * @module visual/time/CurrentTimeElements
 */
...
var Visualizations = function () {
  ...
  function run() {
    ...
    */
    var visualizationElements = document.querySelectorAll(".auma-visualization");
      ...
      if (Object.keys(_config2.default.visualizations.available).length > 0 && visualizationElements.length > 0) {
        ...
        for (var i = 0; i < visualizationElements.length; i++) {
          /*
            ...
            */
          var playlist = visualizationElements[i].getAttribute("data-auma-playlist");
          var song = visualizationElements[i].getAttribute("data-auma-song-index");
    ...
    if (visualizationElements.length > 0) {
      for (var x = 0; x < visualizationElements.length; x++) {
        /*
          ...
          determine which one we run.
        */
        var playlist = visualizationElements[x].getAttribute("data-auma-playlist");
        var song = visualizationElements[x].getAttribute("data-auma-song-index");
...
var SoundCloud = function () {
  ...
  /**
   * Loads the soundcloud SDK for use with auma so the user doesn't have
   * to load it themselves.
   * With help from: http://stackoverflow.com/questions/950087/include-a-javascript-file-in-another-javascript-file
   *
   * @access public
   * @param {object} userConfig 	- The config defined by the user for aumaJS
   */
  ...
  function initSoundcloud() {
    ...
    */
    ...
    VERY important since auma can't stream the copy and pasted
  ...
  function resolveIndividualStreamableURL(url, playlist, index) {
    ...
    SC.get("/resolve/?url=" + url, function (sound) {
      /*
        ...
        so auma can just stream the song normally. We then overwrite
      ...
      */
      if (sound.streamable) {
        ...
        if (playlist != null) {
          aumaHelpers.writeDebugMessage(_config2.default.playlists[playlist].songs[index].name + " by " + _config2.default.playlists[playlist].songs[index].artist + " is not streamable by the Soundcloud API");
        } else {
          /*
          ...
          */
          aumaHelpers.writeDebugMessage(_config2.default.songs[index].name + " by " + _config2.default.songs[index].artist + " is not streamable by the Soundcloud API");
        }
      }
    });
  }
  ...
  function resolveStreamable(url, index) {
    SC.get("/resolve/?url=" + url, function (sound) {
      ...
      */
      ...
      /*
      When all songs are accounted for, then auma is ready
...
var PlaybackSpeedElements = function () {
  ...
  function sync() {
    ...
    */
    var playbackSpeedClasses = document.getElementsByClassName("auma-playback-speed");
    ...
    */
    for (var i = 0; i < playbackSpeedClasses.length; i++) {
      ...
      */
      playbackSpeedClasses[i].classList.remove("auma-playback-speed-10");
      playbackSpeedClasses[i].classList.remove("auma-playback-speed-15");
      playbackSpeedClasses[i].classList.remove("auma-playback-speed-20");
      ...
      */
      switch (_config2.default.playback_speed) {
        case 1:
          playbackSpeedClasses[i].classList.add("auma-playback-speed-10");
          break;
        case 1.5:
          playbackSpeedClasses[i].classList.add("auma-playback-speed-15");
          break;
        case 2:
          playbackSpeedClasses[i].classList.add("auma-playback-speed-20");
          break;
      }
    }
  }
...
var ShuffleElements = function () {
  ...
  function syncMain() {
    ...
    */
    var shuffleButtons = document.getElementsByClassName("auma-shuffle");
    ...
    */
    for (var i = 0; i < shuffleButtons.length; i++) {
      ...
      */
      if (shuffleButtons[i].getAttribute("data-auma-playlist") == null) {
        /*
        If the state of the player is shuffled on, true, then
        we add the 'auma-shuffle-on' class and remove the
        'auma-shuffle-off' class. If the player is not shuffled
        then we do the opposite.
        */
        if (_config2.default.shuffle_on) {
          shuffleButtons[i].classList.add("auma-shuffle-on");
          shuffleButtons[i].classList.remove("auma-shuffle-off");
        } else {
          shuffleButtons[i].classList.add("auma-shuffle-off");
          shuffleButtons[i].classList.remove("auma-shuffle-on");
        }
      }
    }
  }
  ...
  function syncPlaylist(playlist) {
    ...
    */
    var shuffleButtons = document.querySelectorAll('.auma-shuffle[data-auma-playlist="' + playlist + '"]');
...
var SongPlayedProgressElements = function () {
  ...
  function syncGlobal(percentage) {
    ...
    */
    if (!isNaN(percentage)) {
      ...
      */
      var songPlayedProgressBars = document.querySelectorAll(".auma-song-played-progress");

      for (var i = 0; i < songPlayedProgressBars.length; i++) {
        var playlist = songPlayedProgressBars[i].getAttribute("data-auma-playlist");
        var songIndex = songPlayedProgressBars[i].getAttribute("data-auma-song-index");
  ...
  function syncPlaylist(percentage) {
    ...
    */
    if (!isNaN(percentage)) {
      ...
      Get all of the song progress bars
      */
      var songPlayedProgressBars = document.querySelectorAll('.auma-song-played-progress[data-auma-playlist="' + _config2.default.active_playlist + '"]');

      for (var i = 0; i < songPlayedProgressBars.length; i++) {
        var song = songPlayedProgressBars[i].getAttribute("data-auma-song-index");
    ...
    if (_config2.default.active_playlist == null) {
      ...
      */
      if (!isNaN(percentage)) {
        ...
        Get all of the song progress bars
        */
        var songPlayedProgressBars = document.querySelectorAll('.auma-song-played-progress[data-auma-song-index="' + _config2.default.active_index + '"]');
  ...
  function syncSongInPlaylist(percentage) {
    ...
    */
    if (!isNaN(percentage)) {
      ...
      Get all of the song progress bars
      */
      var songPlayedProgressBars = document.querySelectorAll('.auma-song-played-progress[data-auma-playlist="' + _config2.default.active_playlist + '"][data-auma-song-index="' + activePlaylistIndex + '"]');
   ...
   */
  function resetElements() {
    var songPlayedProgressBars = document.getElementsByClassName("auma-song-played-progress");
...
/**
 * aumaJS Checks Module
 * @module utilities/Checks
 */


/**
 * aumaJS Shuffler Module
 * @module utilities/Shuffler
 */


/**
 * aumaJS Events Module
...
/**
 * aumaJS Visualizations Module
 * @module fx/Visualizations
 */


/**
 * aumaJS WaveForm Module
 * @module fx/WaveForm
 */


/**
 * aumaJS Audio Navigation Module.
 * @module utilities/AudioNavigation
 */


/**
 * aumaJS Callbacks Module
 * @module utilities/Callbacks
 */


/**
 * aumaJS Playlists Initializer Module
 * @module init/Playlists
 */


/**
 * Imports the aumaJS Shuffle Elements
 * @module visual/ShuffleElements
 */


/**
 * Imports the aumaJS Mute Elements
 * @module visual/MuteElements
 */


/**
 * Imports the aumaJS Volume Slider
 * @module visual/VolumeSliderElements
 */


/**
 * Imports the aumaJS Time Elements
 * @module visual/TimeElements
 */


/**
 * Imports the aumaJS Play/Pause Elements Module.
 * @module visual/PlayPauseElements
 */


/**
 * Imports the aumaJS MetaData Elements Module.
 * @module visual/MetaDataElements
 */


/**
 * Imports the aumaJS PlaybackSpeedElements Module.
 * @module visual/PlayBackSpeedElements
 */


/**
 * Imports the aumaJS Repeat Element
 * @module visual/RepeatElements
 */
...
/**
 * aumaJS Initializer Module. Helps with the handling of all of the
 * initialization for aumaJS.
 *
...
var Initializer = function () {
   ...
   * auma.init({}) and pass in their settings.
   *
   * Public Accessor: auma.init( user_config_json )
   * @access public
   * @param {object} userConfig - A JSON object of user defined values that help configure and initialize aumaJS.
   */
  function initialize(userConfig) {
    ...
    */
    ...
    event handlers on the auma element and re-bind what is
    necessary.
    */
    ...
    if (userConfig.songs) {
      ...
      */
      if (userConfig.songs.length != 0) {
        ...
        auma for the rest of the configuration.
        */
      ...
      _debug2.default.writeMessage("Please provide a songs object for aumaJS to run!");
    }
    ...
    */
    if (ready) {
      ...
      */
      if (_config2.default.soundcloud_client != "") {
        ...
        /*
        Load up SoundCloud for use with aumaJS.
        */
        _soundcloud2.default.loadSoundCloud(tempUserConfig);
      } else {
        /*
        The user is not using Soundcloud with auma at this point
    ...
    /*
    Debug out what was initialized with aumaJS.
    */
   ...
   *
   * Public Accessor: auma.rebindDisplay()
   * @access public
   */
   ...
   * 		to stream through auma so we get those right away before we
  ...
  function setConfig(userConfig) {
    ...
    */
    ...
    auma sets the active song's volume to the volume defined
    by the user.
    */
...
var WaveForm = function () {
  ...
  */
  ...
  function init() {
    ...
    */
    var waveforms = document.querySelectorAll(".auma-wave-form");
  ...
  function displayWaveForms(svg) {
    var waveformElements = document.querySelectorAll(".auma-wave-form");
    ...
    */
    for (var i = 0; i < waveformElements.length; i++) {
      ...
      */
      var playlist = waveformElements[i].getAttribute("data-auma-playlist");

      /*
        Get the song index attribute of the waveform element.
      */
      var song = waveformElements[i].getAttribute("data-auma-song-index");
...
/**
 * The utility to handle the computation of time in aumaJS.
...
/**
 * aumaJS Visual Handler for Buffered Progress Elements
 *
...
var BufferedProgressElements = function () {
  ...
  function syncGlobal() {
    ...
    */
    var songBufferedProgressBars = document.getElementsByClassName("auma-buffered-progress");
    ...
    */
    for (var i = 0; i < songBufferedProgressBars.length; i++) {
      var playlist = songBufferedProgressBars[i].getAttribute("data-auma-playlist");
      var song = songBufferedProgressBars[i].getAttribute("data-auma-song-index");
  ...
  function syncPlaylist() {
    ...
    Gets all of the song buffered progress bars.
    */
    var songBufferedProgressBarsPlaylist = document.querySelectorAll('.auma-buffered-progress[data-auma-playlist="' + _config2.default.active_playlist + '"]');
    ...
    */
    for (var i = 0; i < songBufferedProgressBarsPlaylist.length; i++) {
      var song = songBufferedProgressBarsPlaylist[i].getAttribute("data-auma-song-index");
  ...
  function syncSong() {
    ...
    Gets all of the song buffered progress bars.
    */
    var songBufferedProgressBarsSongs = document.querySelectorAll('.auma-buffered-progress[data-auma-song-index="' + _config2.default.active_index + '"]');
    ...
    */
    for (var i = 0; i < songBufferedProgressBarsSongs.length; i++) {
      var playlist = songBufferedProgressBarsSongs[i].getAttribute("data-auma-playlist");
  ...
  function syncSongInPlaylist() {
    ...
    Gets all of the song buffered progress bars.
    */
    var songBufferedProgressBarsSongsInPlaylist = document.querySelectorAll('.auma-buffered-progress[data-auma-song-index="' + activePlaylistIndex + '"][data-auma-playlist="' + _config2.default.active_playlist + '"]');
...
/**
 * aumaJS Ended Module. Handles the ended event on the audio.
 *
 * @module events/Ended
 */
...
/**
 * Imports the aumaJS Core Methods
 * @module core/Core
 */
/**
 * Imports the config module
...
/**
 * Imports the aumaJS Play Pause Elements
 * @module visual/PlayPauseElements
 */
...
/**
 * aumaJS Events Module. Handles all of the events we listen to in
 * aumaJS.
 *
 * @module events/Events
 */
...
var Events = function () {
  /**
   * Initializes the handlers for the events listened to by auma
   *
   * @access public
   */
  function initialize() {
    ...
    */
    ...
    /*
    Binds 'auma-play' event handlers
    */
    bindPlay();

    /*
    Binds 'auma-pause' event handlers.
    */
    bindPause();

    /*
    Binds 'auma-play-pause' event handlers.
    */
    bindPlayPause();

    /*
    Binds 'auma-stop' event handlers.
    */
    bindStop();

    /*
    Binds 'auma-mute' event handlers.
    */
    bindMute();

    /*
    Binds 'auma-volume-up' event handlers
    */
    bindVolumeUp();

    /*
    Binds 'auma-volume-down' event handlers
    */
    bindVolumeDown();

    /*
    Binds 'auma-song-slider' event handlers
    */
    bindSongSlider();

    /*
    Binds 'auma-volume-slider' event handlers.
    */
    bindVolumeSlider();

    /*
    Binds 'auma-next' event handlers.
    */
    bindNext();

    /*
    Binds 'auma-prev' event handlers.
    */
    bindPrev();

    /*
    Binds 'auma-shuffle' event handlers.
    */
    bindShuffle();

    /*
    Binds 'auma-repeat' event handlers.
    */
    bindRepeat();

    /*
    Binds 'auma-repeat-song' event handlers.
    */
    bindRepeatSong();

    /*
    Binds 'auma-playback-speed' event handlers.
    */
    bindPlaybackSpeed();

    /*
    Binds 'auma-skip-to' event handlers.
    */
    bindSkipTo();
  ...
  /**
   * Binds click and touchend events for aumaJS play buttons
   *
   * @access private
   */
  function bindPlay() {
    /*
    Gets all of the elements with the class auma-play
    */
    var play_classes = document.getElementsByClassName("auma-play");
  ...
  /**
   * Binds click and touchend events for aumaJS pause buttons.
   *
   * @access private
   */
  function bindPause() {
    /*
    Gets all of the elements with the class auma-pause
    */
    var pause_classes = document.getElementsByClassName("auma-pause");
  ...
  /**
   * Binds click and touchend events for aumaJS play pause buttons
   *
   * @access private
   */
  function bindPlayPause() {
    /*
    Gets all of the elements with the class auma-play-pause
    */
    var play_pause_classes = document.getElementsByClassName("auma-play-pause");
  ...
  /**
   * Binds click and touchend events for aumaJS stop buttons
   *
   * @access private
   */
  function bindStop() {
    /*
    Gets all of the elements with the class auma-stop
    */
    var stop_classes = document.getElementsByClassName("auma-stop");
  ...
  /**
   * Binds click and touchend events for aumaJS mute buttons
   *
   * @access private
   */
  function bindMute() {
    ...
    */
    var mute_classes = document.getElementsByClassName("auma-mute");
  ...
  /**
   * Binds click and touchend events for aumaJS Volume Up Buttons
   *
   * @access private
   */
  function bindVolumeUp() {
    /*
    Gets all of the elements with the class auma-volume-up
    */
    var volume_up_classes = document.getElementsByClassName("auma-volume-up");
  ...
  /**
   * Binds click and touchend events for aumaJS Volume Down Buttons
   *
   * @access private
   */
  function bindVolumeDown() {
    /*
    Gets all of the elements with the class auma-volume-down
    */
    var volume_down_classes = document.getElementsByClassName("auma-volume-down");
  ...
  /**
   * Binds change and input events for aumaJS Song Slider Inputs
   *
   * @access private
   */
  function bindSongSlider() {
    ...
    */
    ...
    /*
    Gets all of the elements with the class auma-song-slider
    */
    var song_sliders = document.getElementsByClassName("auma-song-slider");
  ...
  /**
   * Binds change and input events fro aumaJS Volume Slider inputs
   *
   * @access private
   */
  function bindVolumeSlider() {
    ...
    */
    ...
    /*
    Gets all of the elements with the class auma-volume-slider
        */
    var volume_sliders = document.getElementsByClassName("auma-volume-slider");
  ...
  /**
   * Binds click and touchend events fro aumaJS Next buttons
   *
   * @access private
   */
  function bindNext() {
    /*
    Gets all of the elements with the class auma-next
        */
    var next_classes = document.getElementsByClassName("auma-next");
  ...
  /**
   * Binds click and touchend events for aumaJS prev buttons.
   *
   * @access private
   */
  function bindPrev() {
    /*
    Gets all of the elements with the class auma-prev
    */
    var prev_classes = document.getElementsByClassName("auma-prev");
  ...
  /**
   * Binds click and touchend events for aumaJS shuffle buttons.
   *
   * @access private
   */
  function bindShuffle() {
    /*
    Gets all of the elements with the class auma-shuffle
    */
    var shuffle_classes = document.getElementsByClassName("auma-shuffle");
    ...
    */
    for (var i = 0; i < shuffle_classes.length; i++) {
      ...
      */
      shuffle_classes[i].classList.remove("auma-shuffle-on");
      shuffle_classes[i].classList.add("auma-shuffle-off");
  ...
  /**
   * Binds click and touchend events for aumaJS repeat buttons.
   *
   * @access private
   */
  function bindRepeat() {
    /*
    Gets all of the elements with the class auma-repeat
    */
    var repeat_classes = document.getElementsByClassName("auma-repeat");
    ...
    */
    for (var i = 0; i < repeat_classes.length; i++) {
      ...
      */
      repeat_classes[i].classList.remove("auma-repeat-on");
      repeat_classes[i].classList.add("auma-repeat-off");
  ...
  /**
   * Binds click and touchend events for aumaJS repeat song buttons.
   *
   * @access private
   */
  function bindRepeatSong() {
    /*
    Gets all of the elements with the class auma-repeat-song
    */
    var repeat_song_classes = document.getElementsByClassName("auma-repeat-song");
    ...
    */
    for (var i = 0; i < repeat_song_classes.length; i++) {
      ...
      a state of the repeat control.
      */
      repeat_song_classes[i].classList.remove("auma-repeat-on");
      repeat_song_classes[i].classList.add("auma-repeat-off");
  ...
  /**
   * Binds click and touchend events for aumaJS playback speed buttons
   *
   * @access private
   */
  function bindPlaybackSpeed() {
    /*
    Gets all of the elements with the class auma-playback-speed
    */
    var playback_speed_classes = document.getElementsByClassName("auma-playback-speed");
  ...
  /**
   * Binds click and touchend events for aumaJS skip to buttons.
   *
   * @access private
   */
  function bindSkipTo() {
    /*
    Gets all of the skip to elements with the class 'auma-skip-to'
    */
    var skipToClasses = document.getElementsByClassName("auma-skip-to");
...
/**
 * Imports the aumaJS Visual Mute Elements
 * @module visual/MuteElements
 */
/**
...
/**
 * Imports the aumaJS Visual Volume Slider Elements
 * @module visual/VolumeSliderElements
 */
...
/**
 * Imports the auma Audio Navigation Utility
 * @module utilities/AudioNavigation
 */
...
var Next = function () {
   ...
   *
   * HANDLER FOR:       class="auma-next"
   *
   * GLOBAL:            class="auma-next"
   * PLAYLIST:          class="auma-next" auma-playlist="playlist_key"
   *
   * @access public
   */
  function handle() {
    ...
    */
    if (!_config2.default.is_touch_moving) {
      ...
      */
      var playlist = this.getAttribute("data-auma-playlist");
...
/**
 * Imports the aumaJS Core module.
 * @module core/core
 */
...
var Pause = function () {
   ...
   *
   * HANDLER FOR:       class="auma-pause"
   *
   * GLOBAL:            class="auma-pause"
   * PLAYLIST:          class="auma-pause" auma-playlist="playlist_key"
   * SONG:              class="auma-pause" auma-song-index="song_index"
   * SONG IN PLAYLIST:  class="auma-pause" auma-playlist="playlist-key" auma-song-index="playlist_index"
   *
   * @access public
   */
  function handle() {
    ...
    */
    if (!_config2.default.is_touch_moving) {
      /*
        ...
        class.
      */
      var songIndexAttribute = this.getAttribute("data-auma-song-index");
      var playlistAttribute = this.getAttribute("data-auma-playlist");
...
/**
 * Imports the aumaJS Audio Navigation Utility
 * @module utilities/AudioNavigation
 */
...
var Play = function () {
  /**
   * Handles an event on a play button in auma.
   *
   * HANDLER FOR:       class="auma-play"
   *
   * GLOBAL:            class="auma-play"
   * PLAYLIST:          class="auma-play" auma-playlist="playlist_key"
   * SONG:              class="auma-play" auma-song-index="song_index"
   * SONG IN PLAYLIST:  class="auma-play" auma-playlist="playlist-key" auma-song-index="playlist_index"
   *
   * @access public
   */
  function handle() {
...
/**
 * Imports the aumaJS Checks Utility
 * @module utilities/Checks
 */
...
var PlayPause = function () {
   ...
   *
   * HANDLER FOR:       class="auma-play-pause"
   *
   * GLOBAL:            class="auma-play-pause"
   * PLAYLIST:          class="auma-play-pause" auma-playlist="playlist_key"
   * SONG:              class="auma-play-pause" auma-song-index="song_index"
   * SONG IN PLAYLIST:  class="auma-play-pause" auma-playlist="playlist-key" auma-song-index="playlist_index"
   *
   * @access public
   */
  function handle() {
    ...
    */
    if (!_config2.default.is_touch_moving) {
      ...
      */
      var playlist = this.getAttribute("data-auma-playlist");
      var song = this.getAttribute("data-auma-song-index");
...
/**
 * aumaJS Playback Speed Event Handler
 *
...
var PlaybackSpeed = function () {
   ...
   *
   * HANDLER FOR:       class="auma-playback-speed"
   *
   * @access public
   */
  function handle() {
...
var Prev = function () {
   ...
   *
   * HANDLER FOR:       class="auma-prev"
   *
   * GLOBAL:            class="auma-prev"
   * PLAYLIST:          class="auma-prev" auma-playlist="playlist_key"
   *
   * @access public
   */
  function handle() {
...
/**
 * aumaJS Event Handler for progress
 *
...
var Repeat = function () {
   ...
   *
   * HANDLER FOR:       class="auma-repeat"
   *
   * GLOBAL:            class="auma-repeat"
   * PLAYLIST:          class="auma-repeat" auma-playlist="playlist_key"
   *
   * @access public
   */
  function handle() {
...
/**
 * Handles an event on the auma Repeat Song.
 *
...
var Shuffle = function () {
   ...
   *
   * HANDLER FOR:       class="auma-shuffle"
   *
   * GLOBAL:            class="auma-shuffle"
   * PLAYLIST:          class="auma-shuffle" auma-playlist="playlist_key"
   *
   * @access public
   */
  function handle() {
...
var SkipTo = function () {
   ...
   *
   * HANDLER FOR:       class="auma-skip-to"
   *
   * GLOBAL:            class="auma-skip-to" auma-song-index="song_index" auma-location="seconds"
   * PLAYLIST:          class="auma-skip-to" auma-playlist="playlist_key" auma-song-index="song_index" auma-location="seconds"
   *
   * @access public
   */
  function handle() {
    ...
    */
    if (!_config2.default.is_touch_moving) {
      ...
      */
      var playlist = this.getAttribute("data-auma-playlist");
      var songIndex = this.getAttribute("data-auma-song-index");
      var location = this.getAttribute("data-auma-location");
      ...
      */
      if (location == null) {
        _debug2.default.writeMessage("You must add an 'data-auma-location' attribute in seconds to your 'auma-skip-to' element.");
      }
      ...
      */
      if (songIndex == null) {
        _debug2.default.writeMessage("You must add an 'data-auma-song-index' attribute to your 'auma-skip-to' element.");
      }
...
var SongSlider = function () {
   ...
   *
   * HANDLER FOR:       class="auma-song-slider"
   *
   * GLOBAL:            class="auma-song-slider"
   * PLAYLIST:          class="auma-song-slider" auma-playlist="playlist_key"
   * SONG:              class="auma-song-slider" auma-song-index="song_index"
   * SONG IN PLAYLIST:  class="auma-song-slider" auma-playlist="playlist_key" auma-song-index="song_index"
   *
   * @access public
   */
  function handle() {
...
/**
 * aumaJS Event Handler for Time Update
 *
...
var VolumeDown = function () {
   ...
   *
   * HANDLER FOR:       class="auma-volume-down"
   *
   * @access public
   */
  function handle() {
...
var VolumeSlider = function () {
   ...
   *
   * HANDLER FOR:       class="auma-volume-slider"
   *
   * @access public
   */
  function handle() {
...
var VolumeUp = function () {
   ...
   *
   * HANDLER FOR:       class="auma-volume-up"
   *
   * @access public
   */
  function handle() {
...
/**
 * aumaJS FX Module. Helps with configuring and setting up visualizations
 * and any other features of the Web Audio API that auma takes advantage
 * of.
 *
 * @module fx/FX
 */
var Fx = function () {
  /**
   * Configures the Web Audio API to work with aumaJS
   */
    ...
    */
    if (browserContext) {
      ...
      */
      ...
      aumaHelpers.writeDebugMessage("Web Audio API is unavailable! We will set any of your visualizations with your back up definition!");
    }
  }
...
/**
 * auma should just be an interface to the public functions.
...
var auma = function () {
   ...
   *
   * @access public
   * @param {object} userConfig 	- A JSON object of user defined values that helps configure and initialize aumaJS.
   */
  ...
  /**
   * Returns the current config for aumaJS
   */
  function getConfig() {
   ...
   *
   * Public Accessor: auma.bindNewElements()
   *
   * @access public
   */
   ...
   *
   * Public Accessor: auma.getActivePlaylist()
   *
   * @access public
   */
   ...
   *
   * Public Accessor: auma.getPlaybackSpeed()
   *
   * @access public
   */
   ...
   *
   * Public Accessor: auma.setPlaybackSpeed( speed )
   *
   * @access public
   */
   ...
   *
   * Public Accessor: auma.getRepeat()
   *
   * @access public
   */
  function getRepeat() {
   ...
   *
   * Public Accessor: auma.getRepeatPlaylist()
   *
   * @access public
   */
   ...
   *
   * Public Accessor: auma.getShuffle()
   *
   * @access public
   */
  function getShuffle() {
   ...
   *
   * Public Accessor: auma.getShufflePlaylist( playlist )
   *
   * @access public
   ...
   *
   * Public Accessor: auma.setShuffle()
   *
   ...
   *
   * Public Accessor: auma.setShufflePlaylist( playlist )
   *
   * @access public
   ...
   *
   * Public Accessor: auma.setRepeat()
   *
   * @access public
   ...
   *
   * Public Accessor: auma.setRepeatPlaylist( playlistKey )
   *
   * @access public
   * @param {string} playlist 	- The key representing the playlist ID to to shuffle the playlist.
   ...
   *
   * Public Accessor: auma.setRepeatSong()
   *
   * @access public
   ...
   *
   * Public Accessor: auma.getDefaultAlbumArt()
   *
   * @access public
   */
   ...
   *
   * Public Accessor: auma.getDefaultPlaylistArt()
   *
   * @access public
   */
   ...
   *
   * Public Accessor: auma.setDefaultAlbumArt( url )
   *
   * @access public
   ...
   *
   * Public Accessor: auma.setDefaultPlaylistArt( url )
   *
   * @access public
   ...
   *
   * Public Accessor: auma.getSongPlayedPercentage();
   *
   * @access public
   */
   ...
   *
   * Public Accessor: auma.getSongPlayed();
   *
   * @access public
   */
   ...
   *
   * Public Accessor: auma.setSongPlayedPercentage( float );
   *
   * @access public
   ...
   *
   * Public Accessor: auma.setDebug( bool );
   *
   * @access public
   ...
   * needed.
   *
   * Public Accessor: auma.getActiveSongMetadata();
   *
   * @access public
   ...
   *
   * Public Accessor: auma.getActivePlaylistMetadata();
   *
   * @access public
   ...
   *
   * Public Accessor: auma.getSongAtIndex( song_index )
   *
   * @access public
   ...
   *
   * Public Accessor: auma.getSongAtPlaylistIndex( playlist, index
   *
   * @access public
  ...
  /**
   * Adds a song to the end of the config array.  This will allow auma
   * to play the song in a playlist type setting.
   *
   * Public Accessor: auma.addSong( song_json )
   *
   * @access public
   ...
   * This will allow auma to play the song in a
  ...
  /**
   * Adds a song to a playlist. This will allow auma to play the song in the
   * playlist
   *
   * Public Accessor: auma.addSongToPlaylist( song_json, playlist_key )
   *
   * @access public
   * @param {object} song 			- JSON representation of a song.
   ...
   *
   * Public Accessor: auma.removeSong( index )
   *
   * @access public
   ...
   *
   * Public Accessor: auma.removeSongFromPlaylist( index, playlist )
   *
   * @access public
   ...
   * the visuals.
   *
   * Public Accessor: auma.playNow( song )
   *
   * @access public
   * @param {object} song 	- JSON representation of a song.
   */
   ...
   *
   * Public Accessor: auma.playSongAtIndex( index )
   *
   * @access public
   ...
   * through Javascript. Normally ALL of auma functions that access
   ...
   *
   * Public Accessor: auma.getAnalyser()
   *
   * @access public
   */
   ...
   *
   * Public Accessor: auma.next( playlist );
   *
   * @access public
   ...
   *
   * Public Accessor: auma.prev( playlist );
   *
   * @access public
   * @param {string} [playlist = null] 	- The playlist key
   */
   ...
   *
   * Public Accessor: auma.getSongsInPlaylist( playlist );
   *
   * @access public
   ...
   * songs.
   *
   * Public Accessor: auma.getSongsState();
   *
   * @access public
   */
   ...
   * shuffled songs.
   *
   * Public Accessor: auma.getSongsStatePlaylist( playlist );
   *
   * @access public
   * @param {string} playlist 	- The playlist key
   */
   ...
   *
   * Public Accessor: auma.getActiveIndex()
   *
   * @access public
   */
  ...
  /**
   * Get the version of aumaJS
   *
   * Public Accessor: auma.getVersion()
   *
   * @access public
   */
  function getVersion() {
   ...
   *
   * Public Accessor: auma.getBuffered()
   *
   * @access public
   */
   ...
   * song was playing.
   *
   * Public Accessor: auma.setSongMetaData()
   *
   * @access public
   ...
   *
   * Public Accessor: auma.getPlayerState();
   */
   ...
   * preferences that the user can overwrite similar to auma.
   * Public Accessor: auma.registerVisualization( visualization, preferences )
   *
 ...
 */
/**
 * aumaJS Initializer Module
 *
 * @module init/aumaInitializer
 */
exports.default = auma;
module.exports = exports["default"];
...
/**
 * aumaJS Visual Meta Data Elements Module
 * @module visual/MetaDataElements
 */
...
var PlaylistsInitializer = function () {
   ...
   * to exist in the auma config to be played correctly. If the index
...
/**
 * aumaJS SoundCloud Meta module
 * @module soundcloud/Soundcloud
 */


/**
 * aumaJS Checks Utility.
 * @module utilities/Checks
 */
/**
 * Imports the config module
...
var ContainerElements = function () {
  /**
   * Applies the class 'auma-active-song-container' to the element
  ...
  function setActive(direct) {
    ...
    */
    var songContainers = document.getElementsByClassName("auma-song-container");

    /*
    Removes all of the active song containrs.
    */
    for (var i = 0; i < songContainers.length; i++) {
      songContainers[i].classList.remove("auma-active-song-container");
    }
    ...
    */
    if (_config2.default.active_playlist == "" || _config2.default.active_playlist == null) {
      ...
      */
      ...
      if (document.querySelectorAll('.auma-song-container[data-auma-song-index="' + activeIndex + '"]')) {
        var _songContainers = document.querySelectorAll('.auma-song-container[data-auma-song-index="' + activeIndex + '"]');

        for (var _i = 0; _i < _songContainers.length; _i++) {
          if (!_songContainers[_i].hasAttribute("data-auma-playlist")) {
            _songContainers[_i].classList.add("auma-active-song-container");
          }
        }
      }
    } else {
      ...
      */
      ...
      if (document.querySelectorAll('.auma-song-container[data-auma-song-index="' + activePlaylistIndex + '"][data-auma-playlist="' + _config2.default.active_playlist + '"]')) {
        var _songContainers2 = document.querySelectorAll('.auma-song-container[data-auma-song-index="' + activePlaylistIndex + '"][data-auma-playlist="' + _config2.default.active_playlist + '"]');

        for (var _i2 = 0; _i2 < _songContainers2.length; _i2++) {
          _songContainers2[_i2].classList.add("auma-active-song-container");
        }
      }
    }
  }
...
var CurrentHourElements = function () {
  ...
  function syncGlobal(hours) {
    ...
    */
    var currentHourSelectors = document.querySelectorAll(".auma-current-hours");

    /*
    Set the current hour selector's inner html to hours passed in.
    */
    for (var i = 0; i < currentHourSelectors.length; i++) {
      var playlist = currentHourSelectors[i].getAttribute("data-auma-playlist");
      var songIndex = currentHourSelectors[i].getAttribute("data-auma-song-index");
  ...
  function syncPlaylist(hours) {
    ...
    Get all of the hour selectors
    */
    var currentHourPlaylistSelectors = document.querySelectorAll('.auma-current-hours[data-auma-playlist="' + _config2.default.active_playlist + '"]');

    /*
    Set the current hour selector's inner html to hours passed in.
    */
    for (var i = 0; i < currentHourPlaylistSelectors.length; i++) {
      var songIndex = currentHourPlaylistSelectors[i].getAttribute("data-auma-song-index");
  ...
  function syncSong(hours) {
    if (_config2.default.active_playlist == null) {
      ...
      Get all of the hour selectors
      */
      var currentHourSongSelectors = document.querySelectorAll('.auma-current-hours[data-auma-song-index="' + _config2.default.active_index + '"]');

      /*
      Set the current hour selector's inner html to hours passed in.
      */
      for (var i = 0; i < currentHourSongSelectors.length; i++) {
        var playlist = currentHourSongSelectors[i].getAttribute("data-auma-playlist");
  ...
  function syncSongInPlaylist(hours) {
    ...
    Get all of the hour selectors
    */
    var currentHourPlaylistSongSelectors = document.querySelectorAll('.auma-current-hours[data-auma-playlist="' + _config2.default.active_playlist + '"][data-auma-song-index="' + activePlaylistIndex + '"]');
  ...
  function resetTimes() {
    ...
    */
    var hourSelectors = document.querySelectorAll(".auma-current-hours");
...
var CurrentMinuteElements = function () {
  ...
  function syncGlobal(minutes) {
    ...
    */
    var currentMinuteSelectors = document.querySelectorAll(".auma-current-minutes");

    /*
    Set the current minute selector's inner html to minutes passed in.
    */
    for (var i = 0; i < currentMinuteSelectors.length; i++) {
      var playlist = currentMinuteSelectors[i].getAttribute("data-auma-playlist");
      var songIndex = currentMinuteSelectors[i].getAttribute("data-auma-song-index");
  ...
  function syncPlaylist(minutes) {
    ...
    Get all of the minute selectors
    */
    var currentMinutePlaylistSelectors = document.querySelectorAll('.auma-current-minutes[data-auma-playlist="' + _config2.default.active_playlist + '"]');

    /*
    Set the current minute selector's inner html to minutes passed in.
    */
    for (var i = 0; i < currentMinutePlaylistSelectors.length; i++) {
      var songIndex = currentMinutePlaylistSelectors[i].getAttribute("data-auma-song-index");
  ...
  function syncSong(minutes) {
    if (_config2.default.active_playlist == null) {
      ...
      Get all of the minute selectors
      */
      var currentMinuteSongSelectors = document.querySelectorAll('.auma-current-minutes[data-auma-song-index="' + _config2.default.active_index + '"]');

      /*
      Set the current minute selector's inner html to minutes passed in.
      */
      for (var i = 0; i < currentMinuteSongSelectors.length; i++) {
        var playlist = currentMinuteSongSelectors[i].getAttribute("data-auma-playlist");
  ...
  function syncSongInPlaylist(minutes) {
    ...
    Get all of the minute selectors
    */
    var currentMinutePlaylistSongSelectors = document.querySelectorAll('.auma-current-minutes[data-auma-playlist="' + _config2.default.active_playlist + '"][data-auma-song-index="' + activePlaylistIndex + '"]');
  ...
  function resetTimes() {
    ...
    */
    var minuteSelectors = document.querySelectorAll(".auma-current-minutes");
...
var CurrentSecondElements = function () {
  ...
  function syncGlobal(seconds) {
    ...
    */
    var currentSecondSelectors = document.querySelectorAll(".auma-current-seconds");

    /*
    Set the current second selector's inner html to seconds passed in.
    */
    for (var i = 0; i < currentSecondSelectors.length; i++) {
      var playlist = currentSecondSelectors[i].getAttribute("data-auma-playlist");
      var songIndex = currentSecondSelectors[i].getAttribute("data-auma-song-index");
  ...
  function syncPlaylist(seconds) {
    ...
    Get all of the second selectors
    */
    var currentSecondPlaylistSelectors = document.querySelectorAll('.auma-current-seconds[data-auma-playlist="' + _config2.default.active_playlist + '"]');

    /*
    Set the current second selector's inner html to seconds passed in.
    */
    for (var i = 0; i < currentSecondPlaylistSelectors.length; i++) {
      var songIndex = currentSecondPlaylistSelectors[i].getAttribute("data-auma-song-index");
  ...
  function syncSong(seconds) {
    if (_config2.default.active_playlist == null) {
      ...
      Get all of the second selectors
      */
      var currentSecondSongSelectors = document.querySelectorAll('.auma-current-seconds[data-auma-song-index="' + _config2.default.active_index + '"]');

      /*
      Set the current second selector's inner html to seconds passed in.
      */
      for (var i = 0; i < currentSecondSongSelectors.length; i++) {
        var playlist = currentSecondSongSelectors[i].getAttribute("data-auma-playlist");
  ...
  function syncSongInPlaylist(seconds) {
    ...
    Get all of the second selectors
    */
    var currentSecondPlaylistSongSelectors = document.querySelectorAll('.auma-current-seconds[data-auma-playlist="' + _config2.default.active_playlist + '"][data-auma-song-index="' + activePlaylistIndex + '"]');
  ...
  function resetTimes() {
    ...
    */
    var secondSelectors = document.querySelectorAll(".auma-current-seconds");
...
var CurrentTimeElements = function () {
  ...
  function syncGlobal(time) {
    ...
    */
    var currentTimeSelectors = document.querySelectorAll(".auma-current-time");
    ...
    */
    ...
    for (var i = 0; i < currentTimeSelectors.length; i++) {
      var playlist = currentTimeSelectors[i].getAttribute("data-auma-playlist");
      var songIndex = currentTimeSelectors[i].getAttribute("data-auma-song-index");
  ...
  function syncPlaylist(time) {
    ...
    Get all of the time selectors.
    */
    var currentTimeSelectors = document.querySelectorAll('.auma-current-time[data-auma-playlist="' + _config2.default.active_playlist + '"]');
  ...
  function syncSong(time) {
    if (_config2.default.active_playlist == null) {
      ...
      Get all of the time selectors.
      */
      var currentTimeSelectors = document.querySelectorAll('.auma-current-time[data-auma-song-index="' + _config2.default.active_index + '"]');
  ...
  function syncSongInPlaylist(time) {
    ...
    Get all of the time selectors.
    */
    var currentTimeSelectors = document.querySelectorAll('.auma-current-time[data-auma-playlist="' + _config2.default.active_playlist + '"][data-auma-song-index="' + activePlaylistIndex + '"]');
  ...
  function resetTimes() {
    ...
    */
    var timeSelectors = document.querySelectorAll(".auma-current-time");
...
var DurationCountDownTimeElements = function () {
  ...
  function syncGlobal(timeRemaining) {
    var durationTimeRemainingSelectors = document.querySelectorAll(".auma-time-remaining");

    for (var i = 0; i < durationTimeRemainingSelectors.length; i++) {
      var playlist = durationTimeRemainingSelectors[i].getAttribute("data-auma-playlist");
      var songIndex = durationTimeRemainingSelectors[i].getAttribute("data-auma-song-index");
  ...
  function syncPlaylist(timeRemaining) {
    var durationTimeRemainingSelectors = document.querySelectorAll('.auma-time-remaining[data-auma-playlist="' + _config2.default.active_playlist + '"]');
  ...
  function syncSong(timeRemaining) {
    if (_config2.default.active_playlist == null) {
      var durationTimeRemainingSelectors = document.querySelectorAll('.auma-time-remaining[data-auma-song-index="' + _config2.default.active_index + '"]');
  ...
  function syncSongInPlaylist(timeRemaining) {
    ...
    var durationTimeRemainingSelectors = document.querySelectorAll('.auma-time-remaining[data-auma-playlist="' + _config2.default.active_playlist + '"][data-auma-song-index="' + activePlaylistIndex + '"]');
...
var DurationHourElements = function () {
   ...
   */
  function syncGlobal(hours) {
    var durationHourSelectors = document.querySelectorAll(".auma-duration-hours");

    for (var i = 0; i < durationHourSelectors.length; i++) {
      var playlist = durationHourSelectors[i].getAttribute("data-auma-playlist");
      var songIndex = durationHourSelectors[i].getAttribute("data-auma-song-index");
   ...
   */
  function syncPlaylist(hours) {
    var durationHourSelectors = document.querySelectorAll('.auma-duration-hours[data-auma-playlist="' + _config2.default.active_playlist + '"]');
  ...
  function syncSong(hours) {
    if (_config2.default.active_playlist == null) {
      var durationHourSelectors = document.querySelectorAll('.auma-duration-hours[data-auma-song-index="' + _config2.default.active_index + '"]');
  ...
  function syncSongInPlaylist(hours) {
    ...
    var durationHourSelectors = document.querySelectorAll('.auma-duration-hours[data-auma-playlist="' + _config2.default.active_playlist + '"][data-auma-song-index="' + activePlaylistIndex + '"]');
...
var DurationMinuteElements = function () {
   ...
   */
  function syncGlobal(minutes) {
    var durationMinuteSelectors = document.querySelectorAll(".auma-duration-minutes");

    for (var i = 0; i < durationMinuteSelectors.length; i++) {
      var playlist = durationMinuteSelectors[i].getAttribute("data-auma-playlist");
      var songIndex = durationMinuteSelectors[i].getAttribute("data-auma-song-index");
   ...
   */
  function syncPlaylist(minutes) {
    var durationMinuteSelectors = document.querySelectorAll('.auma-duration-minutes[data-auma-playlist="' + _config2.default.active_playlist + '"]');
  ...
  function syncSong(minutes) {
    if (_config2.default.active_playlist == null) {
      var durationMinuteSelectors = document.querySelectorAll('.auma-duration-minutes[data-auma-song-index="' + _config2.default.active_index + '"]');
  ...
  function syncSongInPlaylist(minutes) {
    ...
    var durationMinuteSelectors = document.querySelectorAll('.auma-duration-minutes[data-auma-playlist="' + _config2.default.active_playlist + '"][data-auma-song-index="' + activePlaylistIndex + '"]');
...
var DurationSecondElements = function () {
   ...
   */
  function syncGlobal(seconds) {
    var durationSecondSelectors = document.querySelectorAll(".auma-duration-seconds");

    for (var i = 0; i < durationSecondSelectors.length; i++) {
      var playlist = durationSecondSelectors[i].getAttribute("data-auma-playlist");
      var songIndex = durationSecondSelectors[i].getAttribute("data-auma-song-index");
   ...
   */
  function syncPlaylist(seconds) {
    var durationSecondSelectors = document.querySelectorAll('.auma-duration-seconds[data-auma-playlist="' + _config2.default.active_playlist + '"]');
  ...
  function syncSong(seconds) {
    if (_config2.default.active_playlist == null) {
      var durationSecondSelectors = document.querySelectorAll('.auma-duration-seconds[data-auma-song-index="' + _config2.default.active_index + '"]');
  ...
  function syncSongInPlaylist(seconds) {
    ...
    var durationSecondSelectors = document.querySelectorAll('.auma-duration-seconds[data-auma-playlist="' + _config2.default.active_playlist + '"][data-auma-song-index="' + activePlaylistIndex + '"]');
...
var DurationTimeElements = function () {
  ...
  function syncGlobal(durationText) {
    var durationTimeSelectors = document.querySelectorAll(".auma-duration-time");

    for (var i = 0; i < durationTimeSelectors.length; i++) {
      var playlist = durationTimeSelectors[i].getAttribute("data-auma-playlist");
      var songIndex = durationTimeSelectors[i].getAttribute("data-auma-song-index");
  ...
  function syncPlaylist(durationText) {
    var durationTimeSelectors = document.querySelectorAll('.auma-duration-time[data-auma-playlist="' + _config2.default.active_playlist + '"]');
  ...
  function syncSong(durationText) {
    if (_config2.default.active_playlist == null) {
      var durationTimeSelectors = document.querySelectorAll('.auma-duration-time[data-auma-song-index="' + _config2.default.active_index + '"]');
  ...
  function syncSongInPlaylist(durationText) {
    ...
    var durationTimeSelectors = document.querySelectorAll('.auma-duration-time[data-auma-playlist="' + _config2.default.active_playlist + '"][data-auma-song-index="' + activePlaylistIndex + '"]');
...
module.exports = {"name":"aumajs","version":"5.3.0","description":"A JavaScript library that allows you to control the design of your media controls in your webpage -- not the browser. No dependencies (jQuery not required) https://521dimensions.com/open-source/aumajs","main":"dist/auma.js","devDependencies":{"babel-core":"^6.26.3","babel-loader":"^7.1.5","babel-plugin-add-module-exports":"0.2.1","babel-polyfill":"^6.26.0","babel-preset-es2015":"^6.18.0","husky":"^1.3.1","jest":"^23.6.0","prettier":"1.15.1","pretty-quick":"^1.11.1","watch":"^1.0.2","webpack":"^2.7.0"},"directories":{"doc":"docs"},"files":["dist"],"funding":{"type":"opencollective","url":"https://opencollective.com/aumajs"},"scripts":{"build":"node_modules/.bin/webpack","prettier":"npx pretty-quick","preversion":"npx pretty-quick && npm run test","postversion":"git push && git push --tags","test":"jest","version":"npm run build && git add -A dist"},"repository":{"type":"git","url":"git+https://github.com/521dimensions/aumajs.git"},"keywords":["webaudio","html5","javascript","audio-player"],"author":"521 Dimensions (https://521dimensions.com)","license":"MIT","bugs":{"url":"https://github.com/521dimensions/aumajs/issues"},"homepage":"https://github.com/521dimensions/aumajs#readme"}
